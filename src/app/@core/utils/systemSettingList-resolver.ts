...
@Injectable()
export class SystemSettingListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    const pagination: Pagination = JSON.parse(localStorage.getItem("SystemSettingList"));
    const pageNumber = pagination ? pagination.currentPage : 1;
    const pageSize = pagination ? pagination.itemsPerPage : 5;
    const filterParams = JSON.parse(localStorage.getItem("SystemSettingListFilter"));
    return this.api.getSystemSettingList(pageNumber, pageSize, filterParams).pipe(
      catchError(error => {
