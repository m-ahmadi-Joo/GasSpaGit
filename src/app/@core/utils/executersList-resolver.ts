...
@Injectable()
export class ExecutersListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    const pagination: Pagination = JSON.parse(localStorage.getItem("ExecutorListPagination"));
    const pageNumber = pagination ? pagination.currentPage : 1;
    const pageSize = pagination ? pagination.itemsPerPage : 5;
    const filterParams = JSON.parse(localStorage.getItem("ExecutorListFilterParams"));
    return this.api.getExecutersList(pageNumber, pageSize, filterParams).pipe(
      catchError(error => {
