...
@Injectable()
export class FileUploaderListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    const pagination: Pagination = JSON.parse(localStorage.getItem("FileUploaderListPagination"));
    const pageNumber = pagination ? pagination.currentPage : 1;
    const pageSize = pagination ? pagination.itemsPerPage : 5;
    const filterParams = JSON.parse(localStorage.getItem("FileUploaderListPagination"));
    return this.api.getFileUploaderList(pageNumber, pageSize, filterParams).pipe(
      catchError(error => {
