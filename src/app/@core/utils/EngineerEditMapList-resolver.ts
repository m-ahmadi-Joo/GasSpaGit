...
@Injectable()
export class EngineerEditMapListResolver
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    ...
    const pagination: Pagination = JSON.parse(localStorage.getItem("EditMapListPagination"));
    const pageNumber = pagination ? pagination.currentPage : 1;
    const pageSize = pagination ? pagination.itemsPerPage : 5;
    const filterParams = JSON.parse(localStorage.getItem("EditMapFilterParams"));
