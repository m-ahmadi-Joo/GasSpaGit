...
@Injectable()
export class GetMoreFiveUnitsListResolver implements Resolve<PaginatedResult<any[]>> {
  constructor(
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    const filterParams = JSON.parse(localStorage.getItem("MorFiveUnitsListFilterParams"));
    this.pagination = JSON.parse(localStorage.getItem("MorFiveUnitsListPagination"));
    ...
    return this.api.getAllMoreFiveUnitsList(pageNumber, pageSize, filterParams).pipe(
      catchError(error => {
