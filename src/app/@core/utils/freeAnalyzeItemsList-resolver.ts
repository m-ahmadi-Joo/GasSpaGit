...
@Injectable()
export class FreeAnalyzeItemsListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    ...
    const pagination: Pagination = JSON.parse(localStorage.getItem("FreeAnalyzeListItemPagination"));
    const pageNumber = pagination ? pagination.currentPage : 1;
    const pageSize = pagination ? pagination.itemsPerPage : 5;
    const filterParams = JSON.parse(localStorage.getItem("FreeAnalyzeListItemPagination"));
    return this.api.getFreeAnalyzeListItems(pageNumber, pageSize , filterParams).pipe(
      catchError(error => {
