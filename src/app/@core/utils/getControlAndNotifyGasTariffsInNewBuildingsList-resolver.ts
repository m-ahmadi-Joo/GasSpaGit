...
@Injectable()
export class GetAllControlAndNotifyGasTariffsInNewBuildingsListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    const pagination: Pagination = JSON.parse(localStorage.getItem("TownListPagination"));
    const pageNumber = pagination ? pagination.currentPage : 1;
    const pageSize = pagination ? pagination.itemsPerPage : 5;
    const filterParams = JSON.parse(localStorage.getItem("TownListPagination"));
    return this.api.ControlAndNotifyGasTariffsInNewBuildingsList(pageNumber, pageSize, filterParams).pipe(
      catchError(error => {
