...
@Injectable()
export class ScanDocumentListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    const pagination: Pagination = JSON.parse(localStorage.getItem("ScanDocumentListPagination"));
    const pageNumber = pagination ? pagination.currentPage : 1;
    const pageSize = pagination ? pagination.itemsPerPage : 5;
    const filterParams = JSON.parse(localStorage.getItem("ScanDocumentListFilterParams"));
    return this.api.getScanDocumentList(pageNumber, pageSize, filterParams).pipe(
      catchError(error => {
