...
@Injectable()
export class HPGasRequestListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  constructor(private api: ApiCommandCenter, private router: Router,
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
        ...
        const filterParams = JSON.parse(localStorage.getItem("GasReqListFilterParams"));
        const hPRequests=true;
        return this.api.getGasRequestList(pageNumber, pageSize , filterParams,hPRequests).pipe(
            catchError(error => {
