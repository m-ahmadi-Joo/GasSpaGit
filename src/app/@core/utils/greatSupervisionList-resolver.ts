...
@Injectable()
export class GreatSupervisionListResolver implements Resolve<PaginatedResult<any[]>> {
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
      const pagination: Pagination = JSON.parse(localStorage.getItem("GreatSupervisionListPagination"));
      const pageNumber = pagination ? pagination.currentPage : 1;
      const pageSize = pagination ? pagination.itemsPerPage : 5;
      // const filterParams = JSON.parse(localStorage.getItem("GreatSupervisionListFilterParams"));
        return this.api.getGreatSupervisionList(pageNumber, pageSize).pipe(
            catchError(error => {
