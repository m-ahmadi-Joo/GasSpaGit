...
@Injectable()
export class ConsultListResolver implements Resolve<PaginatedResult<any[]>> {
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
        const pagination: Pagination = JSON.parse(localStorage.getItem("ConsultListPagination"));
        const pageNumber = pagination ? pagination.currentPage : 1;
        const pageSize = pagination ? pagination.itemsPerPage : 5;
        const filterParams = JSON.parse(localStorage.getItem("ConsultListFilterParams"));
        return this.api.getConsultList(pageNumber, pageSize, filterParams).pipe(
            catchError(error => {
