...
@Injectable()
export class SuppliersListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    const pagination: Pagination = JSON.parse(localStorage.getItem("SupplierListPagination"));
    const pageNumber = pagination ? pagination.currentPage : 1;
    const pageSize = pagination ? pagination.itemsPerPage : 5;
    const filterParams = JSON.parse(localStorage.getItem("SupplierListFilterParams"));
    return this.api.getSuppliersList(pageNumber, pageSize, filterParams).pipe(
      catchError(error => {
