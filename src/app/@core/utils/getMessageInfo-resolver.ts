...
@Injectable()
export class GetMessageInfoResolver implements Resolve<any> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<any> {
    ...
    if (id) {
      //edit
      return this.api.getFrom("Messages", "GetEditInfo/" + id).pipe(
        catchError(error => {
    ...
    return this.api.getFrom("Messages", "GetInfo").pipe(
      catchError(error => {
        this.toastrService.danger('خطا در بازیابی اطلاعات', " ", {
          position: NbGlobalLogicalPosition.TOP_START,
          duration: 5000
