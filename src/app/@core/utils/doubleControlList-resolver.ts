...
@Injectable()
export class DoubleControlListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    const pagination: Pagination = JSON.parse(localStorage.getItem("DoubleControlListPagination"));
    const pageNumber = pagination ? pagination.currentPage : 1;
    const pageSize = pagination ? pagination.itemsPerPage : 5;
    const filterParams = JSON.parse(localStorage.getItem("DoubleControlListFilterParams"));
    return this.api.getDoubleControlList(pageNumber, pageSize, filterParams).pipe(
      catchError(error => {
