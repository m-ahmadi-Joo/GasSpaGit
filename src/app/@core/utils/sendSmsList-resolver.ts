...
@Injectable()
export class SendSmsListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    const pagination: Pagination = JSON.parse(localStorage.getItem("SmsListPagination"));
    const pageNumber = pagination ? pagination.currentPage : 1;
    const pageSize = pagination ? pagination.itemsPerPage : 5;
    const filterParams = JSON.parse(localStorage.getItem("SmsListFilterParams"));
    return this.api.getSmsList(pageNumber, pageSize, filterParams).pipe(
      catchError(error => {
