...
@Injectable()
export class TariffNewBuildingListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    const pagination: Pagination = JSON.parse(localStorage.getItem("TariffNewBuildingListFilterParams"));
    const pageNumber = pagination ? pagination.currentPage : 1;
    const pageSize = pagination ? pagination.itemsPerPage : 5;
    const filterParams = JSON.parse(localStorage.getItem("TariffNewBuildingListFilterParams"));
    return this.api.ControlAndNotifyGasTariffsInNewBuildingsListLoginPage(pageNumber, pageSize, filterParams).pipe(
      catchError(error => {
