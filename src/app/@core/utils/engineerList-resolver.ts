...
@Injectable()
export class EngineerListResolver implements Resolve<PaginatedResult<any[]>> {
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
      const pagination: Pagination = JSON.parse(localStorage.getItem("EngineerListPagination"));
      const pageNumber = pagination ? pagination.currentPage : 1;
      const pageSize = pagination ? pagination.itemsPerPage : 5;
      const filterParams = JSON.parse(localStorage.getItem("EngineerListFilterParams"));
        return this.api.getEngineerList(pageNumber, pageSize, filterParams).pipe(
            catchError(error => {
