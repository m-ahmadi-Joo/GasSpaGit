...
@Injectable()
export class GetAllInspectionTariffsListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    ...
    return this.api.getInspectionTariffsList(pageNumber, pageSize, filterParams).pipe(
      catchError(error => {
