...
@Injectable()
export class AnalyzeListResolver implements Resolve<PaginatedResult<any[]>> {
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
      const pagination: Pagination = JSON.parse(localStorage.getItem("AnalyzeListPagination"));
        const pageNumber = pagination ? pagination.currentPage : 1;
        const pageSize = pagination ? pagination.itemsPerPage : 5;
        const filterParams = JSON.parse(localStorage.getItem("AnalyzeListFilterParams"));
        return this.api.getAnalyzeList(pageNumber,pageSize,filterParams).pipe(
            catchError(error => {
