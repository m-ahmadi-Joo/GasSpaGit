...
@Injectable()
export class AnalyzeListManageListResolver implements Resolve<PaginatedResult<any[]>> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<any[]>> {
    const pagination: Pagination = JSON.parse(localStorage.getItem("AnalyzeListManageList"));
    const pageNumber = pagination ? pagination.currentPage : 1;
    const pageSize = pagination ? pagination.itemsPerPage : 5;
    const filterParams = JSON.parse(localStorage.getItem("AnalyzeListManageList"));
    return this.api.getAnalyzeListConfig(pageNumber, pageSize, filterParams).pipe(
      catchError(error => {
