...
@Component({
  selector: "ngx-map2500HP",
  templateUrl: "./map2500HP.component.html",
  styleUrls: ["./map2500HP.component.scss"],
})
export class Map2500HPComponent implements OnInit {
  fileName: string;
  inputCount = [];
  isEdit: boolean;
  cgmForm: FormGroup;
  ...
  ngOnInit() {
    ...
    this.cgmForm = this.fb.group({
      netWorkSize: ["",Validators.required],
      distanceToEntranceDoor: ["",Validators.required],
      doorToGeneralPublic: ["",Validators.required],
      gasPoint: ["",Validators.required],
      completeNetwork: ["",Validators.required],
      changeNetwork: ["",Validators.required],
      description: [""],
    });
  ...
  submit() {
    this.sendForm = this.fb.group({
      NetworkSize: this.cgmForm.controls.netWorkSize.value,
      GridLineDistanceToEntranceDoor: this.cgmForm.controls
        .distanceToEntranceDoor.value,
      NumberOfDoorToGeneralPublic: this.cgmForm.controls.doorToGeneralPublic
        .value,
      GasConnectionPoint: this.cgmForm.controls.gasPoint.value,
      NeedToCompleteTheNetwork: this.cgmForm.controls.completeNetwork.value,
      RequiresNetWorkSwitching: this.cgmForm.controls.changeNetwork.value,
    ...
    this.api
      .postTo("Map2500", "Map2500Post", this.toFormData(this.sendForm.value))
      .subscribe((res) => {
        if(res) {
          if(res.ok) {
            console.log(res);
