...
import { SystemSettingCustomActionComponent } from "../systemSettingCustomAction/systemSettingCustomAction.component";

@Component({
  selector: "ngx-systemSettingList",
  templateUrl: "./systemSettingList.component.html",
  styleUrls: [
    "../../../forms/../../forms/gasforms/formStyle.scss",
    "./systemSettingList.component.scss",
  ],
})
export class SystemSettingListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.filterParams = JSON.parse(
      localStorage.getItem("inspectionTariffsType")
    );
  ...
  settings = {
    ...
    columns: {
      works: {
        ...
        renderComponent: SystemSettingCustomActionComponent,
  ...
  confirmDelete(row) {
    ...
    this.api.deleteFrom("Base/DeletBaseCity", row.id).subscribe(
      (res: any) => {
        if (res.ok) {
